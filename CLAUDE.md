...
│   │   ├── paths.py           # Writable-data root: CWD from source, %LOCALAPPDATA%\LoreBook frozen
...
│   └── ui/                # PySide6 widgets
│       ├── assets/            # App icon (icon.ico / icon.png, generated by scripts/make_icon.py)
...
│
├── .github/workflows/
│   ├── tests.yml          # CI: ruff + pytest on every push/PR (no TF needed)
│   └── release.yml        # On v* tag: PyInstaller + Inno Setup → installer on GitHub Release
├── lorebook.spec          # PyInstaller build definition (Windows onedir bundle)
├── installer.iss          # Inno Setup script (per-user install, no UAC)
├── configs/
│   └── sort_rules.example.json # Example multi-bin sort rules
├── scripts/
│   ├── make_icon.py            # Regenerate lorebook/ui/assets/icon.ico + icon.png (run once, commit)
...
| File | What it owns |
|------|-------------|
| `lorebook/core/paths.py` | `data_dir()`/`data_path()` — writable-data root (CWD-relative from source; `%LOCALAPPDATA%\LoreBook` when frozen; `LOREBOOK_DATA_DIR` overrides both), `configure_frozen_environment()` (KERAS_HOME) |
...
`lorebook/__init__.py:__version__` is the single source of truth (pyproject.toml reads it via `[tool.setuptools.dynamic]`; the window title shows it). Keep `lorebook/__init__.py` import-light — CI imports it before any dependencies are installed.

To ship a release: bump `__version__`, commit, then `git tag v<version> && git push origin v<version>`. The `release.yml` workflow (windows-latest) verifies the tag matches `__version__`, builds `dist/LoreBook/` with PyInstaller (`lorebook.spec`), compiles `installer.iss` with Inno Setup, and attaches `LoreBook-Setup-<version>.exe` to a GitHub Release.

**Installed-app layout (Windows):** program in `%LOCALAPPDATA%\Programs\Lore Book` (per-user, no UAC); all writable data — `ui_settings.json`, `logs/`, `Card_Images/`, `DBCardCache_*.db`, `*List.csv`, names/prices JSON, Keras weights — in `%LOCALAPPDATA%\LoreBook` (survives uninstall). Path resolution goes through `lorebook/core/paths.py`: from source everything stays CWD-relative exactly as before; `LOREBOOK_DATA_DIR` (set before launch — some constants resolve at import) redirects the data root for testing. When adding a new writable file, route it through `data_path()`.
