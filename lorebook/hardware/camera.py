...
class MockCameraSource(CameraSource):
    ...
    Used for desktop dry runs and tests — no camera needed. Each path's
    basename is preserved on the returned array as the ``source_path``
    attribute so callers can correlate a frame with its file if helpful.
    """
